...
-- ZENITHX COLORS (Black & White)
local COLORS = {
    Primary = Color3.fromRGB(255, 255, 255), -- White
    Secondary = Color3.fromRGB(0, 0, 0),     -- Black
    Accent = Color3.fromRGB(200, 200, 200),  -- Light Gray
    Dark = Color3.fromRGB(20, 20, 20),       -- Dark Gray/Black
    Light = Color3.fromRGB(240, 240, 240),   -- Off-White
    Text = Color3.fromRGB(255, 255, 255),    -- White text
    TextDark = Color3.fromRGB(0, 0, 0),      -- Black text
    Inactive = Color3.fromRGB(40, 40, 40),   -- Dark gray for inactive
}
...
local function saveConfig()
    ...
    -- Save feature states
    pcall(function()
        data.Features.SpeedBoost = SpeedBoostBtn and SpeedBoostBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.AutoSteal = AutoStealBtn and AutoStealBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.BatAimbot = BatAimbotBtn and BatAimbotBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.Galaxy = GalaxyBtn and GalaxyBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.Optimizer = OptimizerBtn and OptimizerBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.AntiRagdoll = AntiRagdollBtn and AntiRagdollBtn.BackgroundColor3 == COLORS.Primary or false
        data.Features.NoAnimations = NoAnimBtn and NoAnimBtn.BackgroundColor3 == COLORS.Primary or false
    end)
    pcall(function()
        writefile("ZenithXDuels_Config.json", HttpService:JSONEncode(data))
    end)
end

local function loadConfig()
    pcall(function()
        if isfile("ZenithXDuels_Config.json") then
            local data = HttpService:JSONDecode(readfile("ZenithXDuels_Config.json"))
            if data.Config then
...
local circleParts = {}
local CIRCLE_COLOR = COLORS.Primary -- White circle
...
ScreenGui.Name = "ZenithXDuelsGUI"
...
MainFrame.BackgroundColor3 = COLORS.Dark -- Black background
...
MainStroke.Color = COLORS.Primary -- White stroke
...
TitleBar.BackgroundColor3 = COLORS.Secondary -- Pure black
...
Title.Text = "ZENITHX DUELS"
Title.Font = Enum.Font.GothamBlack
Title.TextSize = 20
Title.TextColor3 = COLORS.Primary -- White text
...
DiscordLabel.Text = "discord.gg/zenithx"
DiscordLabel.Font = Enum.Font.GothamBold
DiscordLabel.TextSize = 12
DiscordLabel.TextColor3 = COLORS.Accent -- Light gray
...
FeaturesTab.BackgroundColor3 = COLORS.Primary -- White
FeaturesTab.Text = "FEATURES"
FeaturesTab.Font = Enum.Font.GothamBold
FeaturesTab.TextSize = 13
FeaturesTab.TextColor3 = COLORS.TextDark -- Black text on white
...
KeybindsTab.BackgroundColor3 = COLORS.Inactive -- Dark gray
KeybindsTab.Text = "KEYBINDS"
KeybindsTab.Font = Enum.Font.GothamBold
KeybindsTab.TextSize = 13
KeybindsTab.TextColor3 = COLORS.Accent -- Light gray
...
SettingsTab.BackgroundColor3 = COLORS.Inactive -- Dark gray
SettingsTab.Text = "SETTINGS"
SettingsTab.Font = Enum.Font.GothamBold
SettingsTab.TextSize = 13
SettingsTab.TextColor3 = COLORS.Accent -- Light gray
...
MobileTab.BackgroundColor3 = COLORS.Inactive -- Dark gray
MobileTab.Text = "MOBILE"
MobileTab.Font = Enum.Font.GothamBold
MobileTab.TextSize = 11
MobileTab.TextColor3 = COLORS.Accent -- Light gray
...
-- ZENITHX STYLE TOGGLE (Black & White)
local function createZenithXToggle(parent, name, text, yPos)
    local button = Instance.new("TextButton")
    button.Name = name.."Toggle"
    button.Size = UDim2.new(1, -10, 0, 40)
    button.Position = UDim2.new(0, 5, 0, yPos)
    button.BackgroundColor3 = COLORS.Inactive -- Dark gray (inactive)
    button.Text = text
    button.Font = Enum.Font.GothamBold
    button.TextSize = 13
    button.TextColor3 = COLORS.Text -- White text
...
local function createKeybindButton(parent, name, text, currentKey, yPos)
    ...
    container.BackgroundColor3 = COLORS.Dark -- Black background
    ...
    keyButton.BackgroundColor3 = COLORS.Primary -- White
    ...
    keyButton.TextColor3 = COLORS.TextDark -- Black text
    ...
    label.TextSize = 13
    label.TextColor3 = COLORS.Text -- White text
...
local function createNumberInput(parent, name, text, currentValue, min, max, yPos)
    ...
    numButton.BackgroundColor3 = COLORS.Primary -- White
    numButton.Text = tostring(currentValue)
    numButton.Font = Enum.Font.GothamBold
    numButton.TextSize = 12
    numButton.TextColor3 = COLORS.TextDark -- Black text
...
-- Features (Using ZenithX style)
local AutoLeftBtn = createZenithXToggle(FeaturesFrame, "AutoLeft", "Auto Left", 0)
local AutoRightBtn = createZenithXToggle(FeaturesFrame, "AutoRight", "Auto Right", 45)
local SpeedBoostBtn = createZenithXToggle(FeaturesFrame, "SpeedBoost", "Steal Speed", 90)
local AutoStealBtn = createZenithXToggle(FeaturesFrame, "AutoSteal", "Auto Steal", 135)
local BatAimbotBtn = createZenithXToggle(FeaturesFrame, "BatAimbot", "Bat Aimbot", 180)
local GalaxyBtn = createZenithXToggle(FeaturesFrame, "Galaxy", "Jump Power", 225)
local OptimizerBtn = createZenithXToggle(FeaturesFrame, "Optimizer", "Performance", 270)
local AntiRagdollBtn = createZenithXToggle(FeaturesFrame, "AntiRagdoll", "Anti Ragdoll", 315)
local NoAnimBtn = createZenithXToggle(FeaturesFrame, "NoAnimations", "No Animations", 360)

-- Restore saved feature states
task.delay(0.5, function()
    if savedFeatures then
        if savedFeatures.SpeedBoost then
            updateZenithXToggle(SpeedBoostBtn, true)
            startSpeedBoost()
        end
        if savedFeatures.AutoSteal then
            updateZenithXToggle(AutoStealBtn, true)
            task.spawn(function()
                initAutoStealGUI()
                createCircle()
            end)
        ...
        if savedFeatures.BatAimbot then
            updateZenithXToggle(BatAimbotBtn, true)
            startBatAimbot()
        end
        if savedFeatures.Galaxy then
            updateZenithXToggle(GalaxyBtn, true)
            startGalaxy()
        end
        if savedFeatures.Optimizer then
            updateZenithXToggle(OptimizerBtn, true)
            enableOptimizer()
        end
        if savedFeatures.AntiRagdoll then
            updateZenithXToggle(AntiRagdollBtn, true)
            EnableAntiRagdoll()
        end
        if savedFeatures.NoAnimations then
            updateZenithXToggle(NoAnimBtn, true)
...
_ml.TextColor3 = COLORS.Primary; _ml.TextXAlignment = Enum.TextXAlignment.Left
_ml.Parent = MobileFrame

local MobileSupportBtn = createZenithXToggle(MobileFrame, "MobileSupport", "Show Mobile Buttons", 35)
...
_mi.TextColor3 = COLORS.Accent; _mi.TextWrapped = true
...
SaveButton.BackgroundColor3 = COLORS.Primary -- White
SaveButton.Text = "SAVE CONFIG"
SaveButton.Font = Enum.Font.GothamBlack
SaveButton.TextSize = 14
SaveButton.TextColor3 = COLORS.TextDark -- Black text
...
task.spawn(function()
    ...
    while task.wait(5) do
        if AutoStealBtn.BackgroundColor3 == COLORS.Primary then
...
local function initAutoStealGUI()
    ...
    autoStealGui.Name = "ZenithXAutoSteal"
    ...
    frame.BackgroundColor3 = COLORS.Dark -- Black
    ...
    stroke.Color = COLORS.Primary -- White stroke
    stroke.Parent = frame
    ...
    bg.BackgroundColor3 = COLORS.Inactive -- Dark gray
    ...
    fill.BackgroundColor3 = COLORS.Primary -- White
    ...
    radiusText.TextColor3 = COLORS.Primary -- White text
...
-- Toggle Logic
local function updateZenithXToggle(button, active)
    local targetColor = active and COLORS.Primary or COLORS.Inactive
    TweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = targetColor}):Play()
end

AutoLeftBtn.MouseButton1Click:Connect(function()
    local newState = AutoLeftBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(AutoLeftBtn, newState)
    if newState then
        updateZenithXToggle(AutoRightBtn, false)
        leftActive = true
        task.spawn(function()
            moveToTargets(leftTargets)
            leftActive = false
            updateZenithXToggle(AutoLeftBtn, false)
        end)
    else
        leftActive = false
    end
...
AutoRightBtn.MouseButton1Click:Connect(function()
    local newState = AutoRightBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(AutoRightBtn, newState)
    if newState then
        updateZenithXToggle(AutoLeftBtn, false)
        rightActive = true
        task.spawn(function()
...
SpeedBoostBtn.MouseButton1Click:Connect(function()
    local newState = SpeedBoostBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(SpeedBoostBtn, newState)
    if newState then
        startSpeedBoost()
    else
        stopSpeedBoost()
...
AutoStealBtn.MouseButton1Click:Connect(function()
    local newState = AutoStealBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(AutoStealBtn, newState)
    if newState then
        task.spawn(function()
            initAutoStealGUI()
            createCircle()
...
BatAimbotBtn.MouseButton1Click:Connect(function()
    local newState = BatAimbotBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(BatAimbotBtn, newState)
    if newState then
        startBatAimbot()
    else
        stopBatAimbot()
...
GalaxyBtn.MouseButton1Click:Connect(function()
    local newState = GalaxyBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(GalaxyBtn, newState)
    if newState then
        startGalaxy()
    else
        stopGalaxy()
...
OptimizerBtn.MouseButton1Click:Connect(function()
    local newState = OptimizerBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(OptimizerBtn, newState)
    if newState then
        enableOptimizer()
    else
        disableOptimizer()
...
AntiRagdollBtn.MouseButton1Click:Connect(function()
    local newState = AntiRagdollBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(AntiRagdollBtn, newState)
    if newState then
        EnableAntiRagdoll()
    else
        DisableAntiRagdoll()
...
NoAnimBtn.MouseButton1Click:Connect(function()
    local newState = NoAnimBtn.BackgroundColor3 ~= COLORS.Primary
    updateZenithXToggle(NoAnimBtn, newState)
...
-- Main Loops
RunService.Heartbeat:Connect(function()
    if AutoStealBtn.BackgroundColor3 ~= COLORS.Primary or IsStealing then return end
    local hrp = getHRP()
    if not hrp then return end
...
RunService.RenderStepped:Connect(function()
    if AutoStealBtn.BackgroundColor3 ~= COLORS.Primary then return end
    local hrp = getHRP()
    if not hrp then return end
...
player.CharacterAdded:Connect(function(char)
    task.wait(1)
    if AutoStealBtn.BackgroundColor3 == COLORS.Primary then createCircle() end
    if NoAnimBtn.BackgroundColor3 == COLORS.Primary then toggleNoAnimations() end
end)

-- Tab switching
FeaturesTab.MouseButton1Click:Connect(function()
    FeaturesTab.BackgroundColor3 = COLORS.Primary; FeaturesTab.TextColor3 = COLORS.TextDark
    KeybindsTab.BackgroundColor3 = COLORS.Inactive;  KeybindsTab.TextColor3 = COLORS.Accent
    SettingsTab.BackgroundColor3 = COLORS.Inactive;  SettingsTab.TextColor3 = COLORS.Accent
    MobileTab.BackgroundColor3   = COLORS.Inactive;  MobileTab.TextColor3   = COLORS.Accent
...
KeybindsTab.MouseButton1Click:Connect(function()
    KeybindsTab.BackgroundColor3 = COLORS.Primary; KeybindsTab.TextColor3 = COLORS.TextDark
    FeaturesTab.BackgroundColor3 = COLORS.Inactive;  FeaturesTab.TextColor3 = COLORS.Accent
    SettingsTab.BackgroundColor3 = COLORS.Inactive;  SettingsTab.TextColor3 = COLORS.Accent
    MobileTab.BackgroundColor3   = COLORS.Inactive;  MobileTab.TextColor3   = COLORS.Accent
    FeaturesFrame.Visible = false; KeybindsFrame.Visible = true; SettingsFrame.Visible = false; MobileFrame.Visible = false
end)
SettingsTab.MouseButton1Click:Connect(function()
    SettingsTab.BackgroundColor3 = COLORS.Primary; SettingsTab.TextColor3 = COLORS.TextDark
    FeaturesTab.BackgroundColor3 = COLORS.Inactive;  FeaturesTab.TextColor3 = COLORS.Accent
    KeybindsTab.BackgroundColor3 = COLORS.Inactive;  KeybindsTab.TextColor3 = COLORS.Accent
    MobileTab.BackgroundColor3   = COLORS.Inactive;  MobileTab.TextColor3   = COLORS.Accent
...
MobileTab.MouseButton1Click:Connect(function()
    MobileTab.BackgroundColor3   = COLORS.Primary; MobileTab.TextColor3   = COLORS.TextDark
    FeaturesTab.BackgroundColor3 = COLORS.Inactive;  FeaturesTab.TextColor3 = COLORS.Accent
    KeybindsTab.BackgroundColor3 = COLORS.Inactive;  KeybindsTab.TextColor3 = COLORS.Accent
    SettingsTab.BackgroundColor3 = COLORS.Inactive;  SettingsTab.TextColor3 = COLORS.Accent
...
MobileButtonsGui.Name   = "ZenithXMobileButtons"
...
local function createMobileButton(text, position, normalColor, activeColor)
    ...
    btn.TextColor3 = COLORS.TextDark; btn.Font = Enum.Font.GothamBold
    ...
    local s = Instance.new("UIStroke"); s.Color = COLORS.Primary; s.Thickness = 2.5; s.Transparency = 0.3; s.Parent = btn
    MobileButtonColors[btn] = {normal = normalColor, active = activeColor}
    return btn
end

local OFF = COLORS.Inactive
local ON  = COLORS.Primary
...
OpenCloseBtnGui.Name = "ZenithXOpenClose"
...
OpenCloseBtn.BackgroundColor3 = COLORS.Dark
OpenCloseBtn.Text = "⚫"
OpenCloseBtn.TextSize = 26
OpenCloseBtn.Font = Enum.Font.GothamBold
OpenCloseBtn.TextColor3 = COLORS.Primary
...
OpenCloseBtnStroke.Thickness = 2.5; OpenCloseBtnStroke.Color = COLORS.Primary
...
OpenCloseBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(OpenCloseBtnStroke, TweenInfo.new(0.15), {
        Color = MainFrame.Visible and COLORS.Accent or COLORS.Primary
    }):Play()
end)
...
MobileSupportBtn.MouseButton1Click:Connect(function()
    ...
    updateZenithXToggle(MobileSupportBtn, newState)
...
print("╚══════╝╚══════╝╚═╝  ╚═══╝╚═╝   ╚═╝   ╚═╝  ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝╚══════╝╚══════╝")
print("")
print("⚫🤍 ZENITHX DUELS - BLACK & WHITE EDITION 🤍⚫")
print("📱 discord.gg/zenithx")